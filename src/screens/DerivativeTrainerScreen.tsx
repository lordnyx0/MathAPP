...
const DerivativeTrainerScreen: React.FC<DerivativeTrainerScreenProps> = ({ onBack }) => {
    ...
    useEffect(() => {
        ...
        loadHighScore(DERIVATIVE_TRAINER_STATS_KEY).then(hs => {
    ...
    const saveHighScore = async (newScore: number) => {
        const updated = await persistHighScore(DERIVATIVE_TRAINER_STATS_KEY, newScore, highScore);
                        ...
                        <View style={styles.optionsContainer}>
                            {options.map((option, index) => {
                                const state = !showResult
                                    ? 'idle'
                                    : option === currentQuestion?.derivative
                                        ? 'correct'
